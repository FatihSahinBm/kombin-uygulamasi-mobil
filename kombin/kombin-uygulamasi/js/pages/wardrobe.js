        ...
        try {
            ...
            container.innerHTML = items.map(item => `
                <div class="card wardrobe-item-card" data-item="${htmlEscape(JSON.stringify(item))}" style="display: flex; flex-direction: column; position: relative; overflow: hidden; border-radius: 12px; border: none; box-shadow: 0 4px 15px rgba(0,0,0,0.06); transition: transform 0.2s; cursor: pointer;" onmouseover="this.style.transform='translateY(-5px)'" onmouseout="this.style.transform='translateY(0)'">
                    ...
                    <div style="width: 100%; aspect-ratio: 4/5; background: #f8fafc; display: flex; align-items: center; justify-content: center; overflow: hidden; pointer-events: none;">
                        ${item.image_url ?
                    ...
                    <div style="padding: 1rem; text-align: left; background: white; border-top: 1px solid var(--border-color); pointer-events: none;">
                        <p style="margin: 0 0 4px 0; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; color: var(--primary-color);">${formatCategory(item.categories?.name)}</p>
                        <h3 style="margin: 0; font-size: 1.1rem; color: var(--text-color); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${item.name}">${item.name || "İsimsiz Seçim"}</h3>
                        <p style="margin: 5px 0 0 0; color: var(--text-muted); font-size: 0.85rem; font-weight: 500; display: flex; align-items: center; gap: 6px;">

...
export default function LoginScreen({ navigation }) {
  ...
  const handleGoogleAuth = async () => {
    setLoading(true);
    try {
      // Create redirect URL (e.g. kombinapp://auth-callback)
      const redirectUrl = Linking.createURL('auth-callback');
      console.log('Google Auth Redirect URL:', redirectUrl);

      // Start the OAuth flow in Supabase
      ...
      if (result.type === 'success' && result.url) {
        ...
        if (result.url.includes('#')) {
          ...
          const search = result.url.split('?')[1];
        ...
        if (accessToken && refreshToken) {
          ...
          // Fallback to fetch session directly
          const { data: { session } } = await supabase.auth.getSession();
          if (!session) {
            throw new Error('Oturum anahtarları alınamadı.');
          }
        }
      }
    } catch (error) {
      console.log('Google login error:', error);
      Alert.alert('Google Giriş Hatası', error.message || 'Giriş işlemi gerçekleştirilemedi.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.cardSubtitle}>
              {isRegister ? 'Tarzını keşfetmeye başla' : 'Tarzına devam et'}
            </Text>
